...
temperature=Manager().getInstrument('temperature')

# All functions

##
cR=1126 # Coil Resistance
# function giving qubit frequency versus coil drive
def fvsVcoil(V):
	fmax=7.5453
	Voff=-0.440 #Ioff=-0.0089 2.51 mA
	Vperiod=7.61
...
# squareroot of a Lorenzian
def sqrtLorenzian(y0,yAmp,x0,width):
	return lambda x: sqrt(y0+yAmp/(1.0+pow(2.0*(x-x0)/width,2))) 
...
# Lorentzian fit of a dip or peak. Sorted y or sorted or unsorted x y accepted
def fitSqrtLorentzian(y,x='',initialBase='',initialHeight='',initialPosition='',initialWidth='',direction='',debug=False):
	""" fitLorentzian(y,x='',initialBase='',initialHeight='',initialPosition='',initialWidth='',direction='',debug=False) finds the fitting parameters of a lorentzian peak"""
	if debug : print 'Entering fitLorentzian...'
	# generate the x if not defined
	indexes=''			# variable to store the initial order of x values
	...
	if initialWidth=='' or initialWidth==0:
		...
		if initialWidth==0:
			...
			initialWidth=abs(x[index]-x[index-1])/2
	paramGuess=[initialBase,initialHeight,initialPosition,initialWidth]
	sqrtLorentzGuess=sqrtLorenzian(paramGuess[0],paramGuess[1],paramGuess[2],paramGuess[3])
	if debug: 
		print "initial sqrtLorentz guess [y0,yAmp,x0, width] = ",paramGuess 
		figure()
		figNumber=gcf().number
		title('fitSqrtLorentzian')
		pl=plot(x,y,'ro')
		xPlot=linspace(min(x),max(x),500)
		yGuessPlot=sqrtLorentzGuess(xPlot)
		pl1=plot(xPlot,yGuessPlot,'--')
		show()
	# fit
	def fitfunc(x,y0,yAmp,x0,width):
		return sqrtLorenzian(y0,yAmp,x0,width)(x)							#y0+yAmp/(1.0+pow(2.0*(x-x0)/width,2)) 
	paramFit,fitCovariances =scipy.optimize.curve_fit(fitfunc, x, y,p0=paramGuess)
	sqrtLorentzFit=sqrtLorenzian(paramFit[0],paramFit[1],paramFit[2],paramFit[3])
	if debug:  
		print "fit sqrtLorentz result [y0,yAmp,x0, width] = ",paramFit
		figure(figNumber)
		yFitPlot=sqrtLorentzFit(xPlot)
		pl2=plot(xPlot,yFitPlot)
		show()
	if indexes!='': x=[x[i] for i in argsort(indexes)]	# reorder according to inital x if sorted	
	yFitGuess=[sqrtLorentzGuess(xv) for xv in x]
	yFit=[sqrtLorentzFit(xv) for xv in x]
	return paramFit,yFit,yFitGuess
...
def setAWG(sampTime_ns=1,qb_pulse_ns=2000,res_pulse_ns=5000,resDriveLength_ns='',resDriveDelay_ns='',driveReadSep_ns=20,pulseSep_ns=0,triggerInterval=21000,ramsey=False,echoSep1=100,debug=False): #nanoseconds per point
	...
	if qb_pulse_ns<minGateTime:
		markerLength_ns=minGateTime
	else:
...
def qbSpec(nLoops=1,fcenter=1,fspan=1,fstep=0.1,powerQubit=1.,fReadout='',attenRes='',data='',autoPlot=True,autoClear=False,dataSave=True):
	...
	print "f_qb = %f"%x0
	return x0
# measures defined points in a qubit spectrum->NOT READY!!
def qbSpecDiscr(nLoops=1,fStart=5,fStop=6,fstep=0.1,powerQubit=1.,fReadout='',attenRes='',data='',qbGen=True,autoPlot=True,autoClear=False,dataSave=True):
	...
	if dataSave:
		data.savetxt()
#
def qbSpecCoil(nLoops=1,vCoilStart=0, vCoilStop=1,vCoilStep=0.01,fQubit='',powerQubit='',fReadout='',attenRes='',data='',autoPlot=True,autoClear=False,dataSave=True):
	if data=='':	
		data=Datacube('ResQubitCoil')
		DataManager().addDatacube(data)
	if autoPlot:
		data.defineDefaultPlot("vCoil","amp")
		data.defineDefaultPlot("vCoil","ampFit")
		DataManager().addDatacube(data)
		DataManager().autoPlot(data,autoClear)
	#setAWG(sampTime_ns=1,qb_pulse_ns=2000,res_pulse_ns=3000,driveReadSep_ns=20)
	if fReadout!='': mw_cav.setFrequency(fReadout)
	...
	if powerQubit!='': 
		mw_qb.setPower(powerQubit)
		mw_qb.turnOff()
	vCoilLoop=SmartLoop(vCoilStart,vCoilStep,vCoilStop,name='vCoil')
	coil.turnOn()
	time.sleep(1)
	for vCoil in vCoilLoop:
		coil.setVoltage(vCoil)
		[amp,phase]=measureIQ(nLoops=nLoops,averageOverSequence=True,convertToAmpPhase=True)
		data.set(vCoil=vCoil,amp=amp,phase=phase)
		data.commit()
	#coil.turnOff()
	[y0,yAmp,x0,width], ampFit, ampFitGuess = fitLorentzian(data['amp'],x=data['vCoil'])
	#data.createColumn('ampFitGuess',ampFitGuess)
	data.createColumn('ampFit',ampFit)		
	#x0=extremum(data['f_GHz'],data['amp'],minOrMax="min")[0]
	if dataSave:
...
def ramseyMeas(nLoops=1,fQubit='',powerQubit='',fRes='',attenRes='',piPulse=25,detuning='',t1Estim=10000,start=0,stop=1000,step=10,data='',type='normal',echoSep1=1, autoPlot=True,autoClear=False,debug=False,dataSave=False):
	...
	for duration in durations:
		...
		if qbPulse!=piPulse: print 'Alert: pi/2 pulse =',qbPulse,' ns instead of ',piPulse/2,' ns'
		time.sleep(0.1)
		[amp,phase]=measureIQ(nLoops=nLoops,averageOverSequence=True,convertToAmpPhase=True)
...
# qubit spectro
	...
	frequency01=qbSpec(nLoops=5,fcenter=freqCenter_qb,fspan=0.1,fstep=0.001,powerQubit=powerQubitSpectro,fReadout=frequencyRes,attenRes=attenRes,data=qbSpecData)
# Rabi oscillation
	rabiData=Datacube('rabi %f dBm, frequency=%f'%(powerQubitDrive,frequency01))
	data.addChild(rabiData)
	piRabi=rabiMeas(nLoops=5,fQubit=frequency01,powerQubit=powerQubitDrive,fRes=frequencyRes,attenRes=attenRes,start=0,stop=stopRabi,step=stepRabi,data=rabiData)
...
# T2 Ramsey
	...
	t2=ramseyMeas(nLoops=5,fQubit=frequency01,powerQubit=powerQubitDrive,piPulse=piRabi,detuning=detuningRamsey,t1Estim=10000,start=0,stop=stopRamsey,step=stepRamsey,data=t2Data)[1]
...
for coilCurrent in coilCurrents:
	tempStart=temperature.temperature()
	...
	tempStop=temperature.temperature()
		mainData.set(coilCurrent=coilCurrent,frequencyRes=frequencyRes,frequency01=frequency01,piRabi=piRabi,t1=t1,t2=t2,codeExecTime=codeExecTime,tempStart=tempStart,tempStop=tempStop)
		mainData.commit()
...
## Qubit charac vs temperature
mainData=Datacube("Qubit charac vs VCoil 2")                   # main datacube whole flux modulation
DataManager().addDatacube(mainData)
sampTime_ns=2
powerQubitSpectro=-10
powerQubitDrive=0
driveReadSep_ns=20
res_pulse_ns=1500
attenRes=4.5
...
stopRamsey=5000
stepRamsey=10
detuningRamsey=-0.003
firstT1Try=10000
coilCurrent=0.3e-3
repeats=SmartLoop(0.9e-3,0.05e-3,1.5e-3,name="repeats")
...
resSpecData=Datacube('ResoSpec %f V, Icoil=%f'%(attenRes,coil.voltage()))
frequencyRes=resSpec(nLoops=5,fcenter=freqCenter_Res,fspan=0.04,fstep=0.0002,attenRes=attenRes,data=resSpecData,autoClear=False)
...
frequency01=qbSpec(nLoops=5,fcenter=freqCenter_qb,fspan=0.250,fstep=0.0011,powerQubit=powerQubit,fReadout=frequencyRes,attenRes=attenRes,data=qbSpecData,autoClear=False)
...
frequencyRes= 7.094
frequency01= 6.40
vCoilStart=-4.0e-3*cR
vCoilStop=10.0e-3*cR
vCoilStep=0.02e-3*cR
attenRes=3.6

##
setAWG(sampTime_ns=2,qb_pulse_ns=12000,res_pulse_ns=1500,driveReadSep_ns=20,triggerInterval=51000)
qbSpecCoilData=Datacube('coil spectroscopy %s dBm, f=%f GHz'%(powerQubit,frequency01))
iCoilQubit=qbSpecCoil(nLoops=5,vCoilStart=vCoilStart, vCoilStop=vCoilStop,vCoilStep=vCoilStep,fQubit=frequency01,powerQubit='',fReadout=frequencyRes,attenRes=attenRes,data=qbSpecCoilData,autoClear=False)
##
setAWG(sampTime_ns=2,qb_pulse_ns=12000,res_pulse_ns=1500,driveReadSep_ns=20,triggerInterval=51000)
## Rabi oscillation
#frequency01=correct_freq
#frequency01=6.154173
powerQubit=1
rabiData=Datacube('rabi %f dBm, frequency=%f'%(powerQubit,frequency01))
piRabi= rabiMeas(nLoops=20,fQubit=frequency01,powerQubit=powerQubit,fRes=frequencyRes,attenRes=attenRes,start=0,stop=400,step=6,data=rabiData,autoClear=False)
##
piRabi=50.408246
## resonator for qubit in e
setAWG(sampTime_ns=1,qb_pulse_ns=piRabi,res_pulse_ns=1500,driveReadSep_ns=20,triggerInterval=21000)
resSpecEData=Datacube('ResoSpecE %f V, Vcoil=%f'%(attenRes,coil.voltage()))
frequencyResE=resSpec(nLoops=2,fcenter=freqCenter_Res,fspan=0.05,fstep=0.0001,attenRes=attenRes,data=resSpecEData,qbGen=True,fit=True,autoPlot=True,autoClear=False,dataSave=True)

## T1 measurement
#frequency01=6.157276
#powerQubit=0
T1Data=Datacube('t1 %f dBm, frequency=%f, qubitPulse=%f ns'%(powerQubit,frequency01,piRabi))
T1=t1Meas(nLoops=20,targettedPointAccuracy=0.01,firstT1Try=20000,fQubit=frequency01,powerQubit=powerQubit,fRes=frequencyRes,attenRes=attenRes,piRabi=piRabi,data=T1Data,autoPlot=True,autoClear=False,debug=True)

## T2 Ramsey
#frequency01=6.157172
#frequency01=correct_freq
#powerQubit=0
#piRabi=piRabi
t2Data=Datacube('t2 %f dBm, frequency=%f, qubitPulse=%f ns'%(powerQubit,frequency01,piRabi)) 
t2=ramseyMeas(nLoops=20,fQubit=frequency01,powerQubit=powerQubit,piPulse=piRabi,detuning=-0.002,t1Estim=10000,start=0,stop=20000,step=20,data=t2Data,dataSave=True)[1]
##
print mw_qb.frequency()
correct_freq=mw_qb.frequency()+1.0/479
print correct_freq
## T2 Ramsey Echo
...
ramseyMeas(nLoops=20,fQubit=frequency01,powerQubit=powerQubit,piPulse=piRabi,detuning=-0.002,t1Estim=10000,start=piRabi,stop=28000,step=200,type='echo',data=t2DataHahnEcho,dataSave=True)
...
print t1
## fit echo
[y0,yAmp,x0,t1],yFit,yFitGuess=fitT1(data['amp'],data['duration'],initialY0=0.008,initialYAmp=0.004,initialX0=0,initialT1=10000,debug=True)
print t1
...
powerQubit=-21
##qubit spectroscopy in microwave frequency
for i in arange(280,420,10):
	setAWG(sampTime_ns=1,qb_pulse_ns=28,res_pulse_ns=1500,ramsey='normal',triggerInterval=21000,pulseSep_ns=i,debug=False)
	time.sleep(2)
##
setAWG(sampTime_ns=1,qb_pulse_ns=piRabi/2,res_pulse_ns=1500,driveReadSep_ns=20,triggerInterval=105000)
##
print measureIQ(nLoops=5,averageOverSequence=True,convertToAmpPhase=True)[0]
##
mw_qb.setFrequency(6.157294)
##
data=Datacube('Test')
DataManager().addDatacube(data)
data.defineDefaultPlot("index","amp")
DataManager().addDatacube(data)
DataManager().autoPlot(data,False)
for i in range(100):
	amp=measureIQ(nLoops=5,averageOverSequence=True,convertToAmpPhase=True)[0]
	data.set(index=i,amp=amp)
	data.commit()
data.savetxt()
##
print frequency01
