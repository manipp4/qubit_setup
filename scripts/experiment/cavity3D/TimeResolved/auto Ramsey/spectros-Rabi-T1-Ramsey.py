...
def fvsIcoil(I):
	fmax=7.5453
	Ioff=(-1.470)*1.e-3 #Ioff=-0.0089 2.51 mA
	Iperiod=(6.685)*1.e-3
...
## set the awg
##
print setAWG(sampTime_ns=1,qb_pulse_ns=1000,res_pulse_ns=3500,pulseSep_ns=2000,driveReadSep_ns=20,ramsey='echo')
##
def setAWG(sampTime_ns=1,qb_pulse_ns=2000,res_pulse_ns=5000,driveReadSep_ns=20,pulseSep_ns=0,triggerInterval=21000,ramsey=False,debug=False): #nanoseconds per point
	...
	if ramsey=='normal' or ramsey=='echo':
		print 'ramsey pulse'
...
def setAWGPulses(sampTime_ns=1,triggerInterval=21000,debug=False,markerForswitch=False,switchMargin=100,sequence=''):
	ns=1.0e-9
	nPoints=20000
	samplRate=1.0/sampTime_ns/ns
	reprate=samplRate/nPoints
	AWG.setRepetitionRate(reprate)
	AWG.setTriggerInterval(triggerInterval*ns)# has to be specidfied in seconds
	pg_qb.clearPulse()
	currentTime=0
	if sequence!='':
		for item in sequence:
			duration_ns=item[0]
			duration_pts=round(duration_ns/sampTime_ns)
			level=item[1]
			if level!=0:
				pg_qb.generatePulse(duration=duration_pts, frequency=mw_qb.frequency(), DelayFromZero=currentTime,useCalibration=False)	

def setAWGRamsey(sampTime_ns=1,qb_pulse_ns=2000,res_pulse_ns=5000,driveReadSep_ns=20,triggerInterval=21000,pulseSep_ns=0,debug=False,switch=True): #nanoseconds per point
	switchMargin_ns=500
	...
	qbPulse_pts=abs(round(qb_pulse_ns/sampTime_ns))
	resPulse_pts=abs(round(res_pulse_ns/sampTime_ns))
	driveReadSep_pts=abs(round(driveReadSep_ns/sampTime_ns))
	pulseSep_pts=abs(round(pulseSep_ns/sampTime_ns))
	...
	DelayFromZeroReadout_pts=20000-resPulse_pts
	DelayFromZeroPulse2_pts=DelayFromZeroReadout_pts-driveReadSep_pts-qbPulse_pts
	DelayFromZeroPulse1_pts=DelayFromZeroPulse2_pts-pulseSep_pts-qbPulse_pts
	markerStart2=20000-(res_pulse+driveReadSep+qb_pulseHalf+markerLengthHalf+markerDelay)
	markerStop2=20000-(res_pulse+driveReadSep+qb_pulseHalf-markerLengthHalf+markerDelay)
	markerStart1=20000-(res_pulse+driveReadSep+qb_pulseHalf+markerLengthHalf+markerDelay-pulseSep_pts-qbPulse_pts)
	markerStop1=20000-(res_pulse+driveReadSep+qb_pulseHalf-markerLengthHalf+markerDelay-pulseSep_pts-qbPulse_pts)
	
	pg_qb.clearPulse()
	pg_qb.generatePulse(duration=qbPulse_pts, frequency=mw_qb.frequency(), DelayFromZero=DelayFromZeroPulse1_pts,useCalibration=False)
	pg_qb.generatePulse(duration=qbPulse_pts, frequency=mw_qb.frequency(), DelayFromZero=DelayFromZeroPulse2_pts,useCalibration=False)
	pg_qb.generateMarker(name='marker3',start=markerStart1,stop=markerStop1,level=3,start2=markerStart2,stop2=markerStop2)
	pg_qb.sendPulse(markersName='marker3')
	
	pg_cav.clearPulse()
	pg_cav.generatePulse(duration=resPulse_pts, frequency=mw_cav.frequency(), DelayFromZero=DelayFromZeroReadout_pts,useCalibration=False)
	pg_cav.generateMarker(name='marker1',start=DelayFromZeroReadout_pts-1000,stop=DelayFromZeroReadout_pts,level=3)
	pg_cav.sendPulse(markersName='marker1')
	
	pg_ch2.clearPulse()
	pg_ch2.generatePulse(duration=resPulse_pts, frequency=mw_cav.frequency(), DelayFromZero=DelayFromZeroReadout_pts,useCalibration=False)
	pg_ch2.generateMarker(name='marker2',start=DelayFromZeroReadout_pts-1000,stop=DelayFromZeroReadout_pts,level=3)
	pg_ch2.sendPulse(markersName='marker2')
	return qbPulse_pts*sampTime_ns, resPulse_pts*sampTime_ns, driveReadSep_pts*sampTime_ns,pulseSep_pts*sampTime_ns
...
#
def ramseyMeas(nLoops=1,fQubit='',powerQubit='',fRes='',attenRes='',piPulse=25,detuning='',t1Estim=10000,start=0,stop=1000,step=10,data='',type='normal', autoPlot=True,autoClear=False,debug=False,dataSave=False):
	if data=='':	
	...
	for duration in durations:
		...
		[qbPulse, resPulse,driveReadSep,pulseSep]=setAWG(sampTime_ns=sampTime_ns,qb_pulse_ns=piPulse,res_pulse_ns=1500,ramsey=type,triggerInterval=triggerInterval,pulseSep_ns=duration,debug=debug)
...
def measureAtAWorkingPoint(coilCurrent=0,freqCenter_qb='',attenRes=4.5,powerQubitSpectro=0,powerQubitDrive=10,firstT1Try=10000,stopRabi=100,stepRabi=4,stopRamsey=15000,stepRamsey=100,targettedPointAccuracy=0.01,data='',detuningRamsey=-0.001):
	start = time.clock()
...
# qubit spectro
	...
	frequency01=qbSpec(nLoops=5,fcenter=freqCenter_qb,fspan=0.15,fstep=0.001,powerQubit=powerQubitSpectro,fReadout=frequencyRes,attenRes=attenRes,data=qbSpecData)
# Rabi oscillation
...
# T2 Ramsey
	...
	data.addChild(t2EchoData)
	t2Echo=ramseyMeas(nLoops=20,fQubit=frequency01,powerQubit=powerQubitDrive,piPulse=piRabi,detuning=detuningRamsey,t1Estim=10000,start=0,stop=stopRamsey,step=stepRamsey,type='echo',data=t2Data)[1]
	codeExecTime =time.clock()-start
	data.set(frequencyRes=frequencyRes,frequency01=frequency01,piRabi=piRabi,t1=t1,t2Echo=t2Echo,codeExecTime=codeExecTime)
	data.commit()
	data.savetxt()
	print 'done in ',codeExecTime, 's.'
	return [frequencyRes,frequency01,piRabi,t1,t2,t2Echo,codeExecTime]

##
measureAtAWorkingPoint(coilCurrent=0.18e-3,attenRes=4.5,powerQubitSpectro=-5,powerQubitDrive=1,firstT1Try=10000,stopRabi=80,stepRabi=2,stopRamsey=15000,stepRamsey=100,targettedPointAccuracy=0.01,data='')
setAWG(sampTime_ns=1,qb_pulse_ns=2000,res_pulse_ns=5000,driveReadSep_ns=20,triggerInterval=21000,debug=False)

##
for i in range(5):
	measureAtAWorkingPoint(coilCurrent=-1.47e-3,attenRes=4.5,powerQubitSpectro=-10,powerQubitDrive=5,firstT1Try=10000,stopRabi=80,stepRabi=2,stopRamsey=15000,stepRamsey=100,targettedPointAccuracy=0.01,data='')
setAWG(sampTime_ns=1,qb_pulse_ns=2000,res_pulse_ns=5000,driveReadSep_ns=20,triggerInterval=21000,debug=False)
for i in range(5):
	measureAtAWorkingPoint(coilCurrent=1.1e-3,attenRes=4.5,powerQubitSpectro=0,powerQubitDrive=13,firstT1Try=10000,stopRabi=80,stepRabi=2,stopRamsey=15000,stepRamsey=100,targettedPointAccuracy=0.01,data='')
setAWG(sampTime_ns=1,qb_pulse_ns=2000,res_pulse_ns=5000,driveReadSep_ns=20,triggerInterval=21000,debug=False)
## CODE DV
mainData=Datacube("Qubit charac vs iCoil 3")                   # main datacube whole flux modulation
...
coilCurrents=SmartLoop(0.1e-3,0.1e-3,0.9e-3,name="coilCurrents")
...
## SET AWG
setAWG(sampTime_ns=2,qb_pulse_ns=0,res_pulse_ns=3500,driveReadSep_ns=20)


## Set working point and parameters
frequencyResBare=7.080
spanRes=0.03
coilCurrent=(1.1)*1e-3
coil.setCurrent(coilCurrent)
coil.turnOn()
...
print freqCenter_qb
##
freqCenter_Res=7.089900
## Resonator line
setAWG(sampTime_ns=2,qb_pulse_ns=12000,res_pulse_ns=2500,driveReadSep_ns=20,triggerInterval=51000)
resSpecData=Datacube('ResoSpec %f V, Icoil=%f'%(attenRes,coil.current()))
frequencyRes=resSpec(nLoops=5,fcenter=freqCenter_Res,fspan=0.08,fstep=0.0005,attenRes=attenRes,data=resSpecData,autoClear=True)
...
frequency01=qbSpec(nLoops=5,fcenter=freqCenter_qb,fspan=0.05,fstep=0.0002,powerQubit=powerQubit,fReadout=frequencyRes,attenRes=attenRes,data=qbSpecData,autoClear=False)
...
frequencyRes= 7.090099
frequency01= 6.350671
##
setAWG(sampTime_ns=2,qb_pulse_ns=12000,res_pulse_ns=1500,driveReadSep_ns=20,triggerInterval=51000)
qbSpecCoilData=Datacube('coil spectroscopy %s dBm, f=%f GHz'%(powerQubit,frequency01))
iCoilQubit=qbSpecCoil(nLoops=5,iCoilStart=-2.4e-3, iCoilStop=4e-3,iCoilStep=0.01e-3,fQubit=frequency01,powerQubit='',fReadout=frequencyRes,attenRes=attenRes,data=qbSpecCoilData,autoClear=False)

## Rabi oscillation
powerQubit=1
rabiData=Datacube('rabi %f dBm, frequency=%f'%(powerQubit,frequency01))
piRabi= rabiMeas(nLoops=10,fQubit=frequency01,powerQubit=powerQubit,fRes=frequencyRes,attenRes=attenRes,start=0,stop=200,step=10,data=rabiData,autoClear=False)
##
piRabi=62
## T1 measurement
powerQubit=1
T1Data=Datacube('t1 %f dBm, frequency=%f, qubitPulse=%f ns'%(powerQubit,frequency01,piRabi))
T1=t1Meas(nLoops=20,firstT1Try=10000,fQubit=frequency01,powerQubit=powerQubit,fRes=frequencyRes,attenRes=attenRes,piRabi=piRabi,data=T1Data,autoPlot=True,autoClear=False,debug=True)

## T2 Ramsey
powerQubit=1
piRabi=62
t2Data=Datacube('t2 %f dBm, frequency=%f, qubitPulse=%f ns'%(powerQubit,frequency01,piRabi)) 
t2=ramseyMeas(nLoops=20,fQubit=frequency01,powerQubit=powerQubit,piPulse=piRabi,detuning=-0.002,t1Estim=10000,start=0,stop=5000,step=20,data=t2Data)[1]
...
setAWG(sampTime_ns=3,qb_pulse_ns=350,res_pulse_ns=1500,driveReadSep_ns=20,triggerInterval=21000,debug=False)
...
#print mw_qb.read()
##
print measureIQ(nLoops=10,averageOverSequence=True,convertToAmpPhase=True)
