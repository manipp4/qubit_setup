...
attenuator=Manager().getInstrument('Yoko2')
...
def setAWGV2(sampTime_ns=1,triggerInterval=21000,ch1=True,ch1_pulseType='readout',ch1_pulse_ns=2500,driveReadSep_ns=20,ch2=False,ch3=True,ch3_pulseType='gaussian',sigma_ns=20,ch4=False,debug=False): #nanoseconds per point
	samplRate=1.e9/sampTime_ns
	reprate=samplRate/20000.
	AWG.setRepetitionRate(reprate)
	AWG.setTriggerInterval(triggerInterval*1e-9)# has to be specidfied in seconds
	...
	if ch1:
		if ch1_pulseType=='readout':
			...
			wf1=waveGen.readoutWaveform(ch1_pulse)
			...
			ch1_pulse=abs(round(ch1_pulse_ns/sampTime_ns))
			wf1=waveGen.readoutSwitchWaveform(ch1_pulse,switchRiseBuffer=switchRiseBuffer)
			waveGen.send2AWG(wf1)
			waveGen.loadWF2Channel(1,wf1[2])
		else:
...
#
##
setAWGV2(sampTime_ns=1,triggerInterval=21000,ch1=True,ch1_pulseType='readout',ch1_pulse_ns=2500,driveReadSep_ns=20,ch2=False,ch3=True,ch3_pulseType='gaussian',sigma_ns=2000,ch4=False,debug=False)
##
...
## Main section
mainData=Datacube("Qubit charac vs VCoil 6")                   # main datacube whole flux modulation
...
coilVoltages=SmartLoop(-2.5,0.5,2.5,name="coilVoltages")
for coilVoltage in coilVoltages:
	onePointData=Datacube("coilVoltage=%f"%coilVoltage)    #create datacube a child for this flux
	mainData.addChild(onePointData)
  	print "voltage= %f"%coilVoltage
	try:		
		params0=measureAtAWorkingPointV2(doResSpec=doResSpec,data=onePointData,coilVoltage=coilVoltage,attenRes=attenRes,doQbSpec=doQbSpec,freqCenter_qb='',powerQubitSpectro=powerQubitSpectro,doRabi=doRabi,powerQubitDrive=10,stopRabi=100,stepRabi=4,doT1=doT1,firstT1Try=10000,targettedPointAccuracy=0.01,doRamsey=doRamsey,stopRamsey=15000,stepRamsey=100,detuningRamsey=-0.001)
		
		#this assumes that a T1 has been done before a Ramsey !!!
		params1=params0+[0]*(7-len(params0))
		mainData.set(coilVoltage=coilVoltage,frequencyRes=params1[1],frequency01=params1[2],piRabi=params1[3],t1=params1[4],t2=params1[5],codeExecTime=params1[0]) 
...
frequencyResBare=7.3088
spanRes=0.03
coilCurrent=6
coil.setVoltage(coilCurrent)
coil.turnOn()
freqCenter_qb=fvsIcoil(coilCurrent)          		  # calculated first parameters 
chi=0.155**2/(freqCenter_qb-frequencyResBare)
freqCenter_Res=frequencyResBare-chi
span01=0.1
powerQubit=5
T1est=10000
...
setAWGV2(sampTime_ns=2,triggerInterval=51000,ch1=True,ch1_pulseType='readout_switch',ch1_pulse_ns=2500,driveReadSep_ns=20,ch3=False,ch3_pulseType='gaussian',sigma_ns=2000)
...
frequencyRes=resSpec(nLoops=40,fcenter=freqCenter_Res,fspan=0.05,fstep=0.0002,attenRes=attenRes,data=resSpecData,autoClear=False)
...
#powerQubit=-10
piRabi=3000
#freqCenter_qb=5.88+0.36/2
##qubit spectroscopy in microwave frequency
setAWGV2(sampTime_ns=2,triggerInterval=51000,ch1=True,ch1_pulseType='readout_switch',ch1_pulse_ns=2500,driveReadSep_ns=20,ch3=True,ch3_pulseType='gaussian',sigma_ns=3000)
#setAWG(sampTime_ns=2,qb_pulse_ns=12000,res_pulse_ns=2500,driveReadSep_ns=20,triggerInterval=51000)
qbSpecData=Datacube('spectroscopy01 %f dBm, vcoil=%f'%(powerQubit,coil.voltage()))
frequency01=qbSpec(nLoops=15,fcenter=freqCenter_qb,fspan=0.1,fstep=0.0002,powerQubit=powerQubit,fReadout=frequencyRes,attenRes=attenRes,data=qbSpecData,autoClear=False)

##
freqCenter_qb=5.41213
powerQubit=-10
