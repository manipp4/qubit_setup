...
#######################################################################
mainData=Datacube("Spec vs coilCurrent 5")
...
for coilCurrent in coilCurrents:
	...
	frequencyLoop=SmartLoop(frequencyRes-0.015,0.0003,frequencyRes+0.015,name='frequency')
	coil.turnOn()
	time.sleep(1)
	...
	onePointData.addChild(data)
	frequencyLoop=SmartLoop(frequency01-span01/2,0.001,frequency01+span01/2,name='frequency')
	coil.turnOn()
	time.sleep(1)
	for i in range(10):
		for f in frequencyLoop:
