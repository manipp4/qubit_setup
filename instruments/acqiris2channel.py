...
class Instr(Instrument):
  ...
  def initialize(self,*args,**kwargs): 
    ...
    self._params["fullScales"] = [5.0] * 2
    self._params["offsets"] = [0.0] * 2
    self._params["couplings"] = [3] * 2
    self._params["bandwidths"] = [3] * 2
    ...
    self.lastWaveformArraySizes=zeros(2,dtype=int32)	# number of samples in each waveform channels
    self.lastWaveformArray = zeros((2,1))		# waveform (full sequence) array
    ...
    self.lastNbrSegmentsArray=zeros(2,dtype=int32)# number of segments actually returned for each channel in the last transfer
    ...
    self.lastWave['lastAverageArray']=zeros((2,1))
  ...
  CAL_FAST=2               # calibrate specified channels  
  def CalibrateV1(self,option=CAL_FAST,channels=3):
    """
  ...
  def AcquireTransferV4(self, voltages=True, wantedChannels=3, transferAverage=False, getHorPos=True, getTimeStamps=True,nLoops=1.,timeOut=10):
    self.nLoops=nLoops
    ...
    totalArraySizes=zeros(2,dtype=int32)
    transferAverages=zeros(2,dtype=bool)
    transferAverages[:]=[transferAverage]*2
    ...
    self.lastWaveformArray=zeros((2,size),dtype=float64)      
    self.lastWaveformArraySizes=zeros(2,dtype=int32)
    self.lastWaveformArraySizes[:]=[size]*2
    self.lastAverageCalculated=False
    self.lastWave['averageCalculated']=self.lastAverageCalculated
    self.lastWave['lastAverageArray']=zeros((2,1))                 
    ...
    nbrSegmentsArray=zeros(2,dtype=int32) 
  ...
  def DMATransferV1(self,voltage = True,getTimeStamps = False,transferAverage = False, *args):        
    ...
    if transferAverage == False:
      self.wf = zeros((2,self._params["numberOfPoints"] * self._params["numberOfSegments"]))
    else:
      self.wf = zeros((2,self._params["numberOfPoints"]))  
    ...
    c_transferAverageType = c_bool * 2
    c_transferAverageArray = c_transferAverageType(*[transferAverage]*2)
    number_of_segments = c_int32(self._params["numberOfSegments"])
    number_of_samples_per_segment = c_int32(self._params["numberOfPoints"])
    number_of_segments_array = zeros(2,dtype = int32)
    number_of_segments_array[:] = self._params["numberOfSegments"]
    number_of_samples_per_segment_array = zeros(2,dtype = int32)
    number_of_samples_per_segment_array[:] = self._params["numberOfPoints"]
    sampling_intervals = zeros(2,dtype = float64)
    ...
    status=self.__acqiris_QuantroDLL1.DMATransferV1(
      ...
      self.wf[0,:].ctypes.data,self.wf[1,:].ctypes.data,
      sampling_intervals.ctypes.data,
      byref(time_stamps_1),byref(time_stamps_2),
      byref(c_time_us)
      )
    ...
    if status != 0:
      ...
      self.notify("data",(list(self.waveform_1),list(self.waveform_2))
