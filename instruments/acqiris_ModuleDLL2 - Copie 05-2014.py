...
#      pass
  ...
  def getCorrections(self, frequency):
    print round(frequency,4), [round(f,4) for f in self.corrections.keys()]
    xOffset=0.
    yOffset=0.
    angle=0.
...
#    self.AcquireTransferV4(voltages=True, wantedChannels=15, transferAverage=False, getHorPos=True, getTimeStamps=False,nLoops=nLoops)
    ...
    components=zeros((lenArray,2,self.parent.getLastWave()  ['nbrSegmentArray'][0]))
    rotatedComponents=zeros((lenArray,2,self.parent.getLastWave()['nbrSegmentArray'][0]))
    
    #Array for clicks
    clicks=zeros((lenArray,self.parent.getLastWave()['nbrSegmentArray'][0]))
