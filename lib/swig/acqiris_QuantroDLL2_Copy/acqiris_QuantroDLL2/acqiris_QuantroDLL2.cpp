// VS 2012 corrected DV 2014
// boxcar functionality added to demodulation with parameters startInSegment stopInSegment 
...
void Demodulator::demodulate(bool twoCh,double *ChA, double *ChB, double *horPos, int nbrSegments,int nbrSamplesPerSegment,int startInSegment,int stopInSegment ,double samplingTime,double frequency,double phase,int intervalMode,bool tryCorrec,bool averageOnly,bool APhi,int *nbrIntervalsPerSegment,double *array1,double *array2,double *arrayMean1,double *arrayMean2)
{	// cout << "PI = " << PI << endl;
	int i,j,p,q,r,nbrSamplesPerInterval,nbrSamplesPerSubSegment,index=-1;
	double arg,deltaArg,I,Q,x,y,det;

	// check start stop and return if invalid 
	*nbrIntervalsPerSegment=0;						// in case it fails, no demodulation => nbrIntervalsPerSegment=0
	if (startInSegment <0) startInSegment=nbrSamplesPerSegment+startInSegment;
	if (stopInSegment <0) stopInSegment=nbrSamplesPerSegment+stopInSegment;
	if ((startInSegment>stopInSegment) || (startInSegment<0) || (stopInSegment>nbrSamplesPerSegment-1))
	{	cout<<"cannot use negative absolute indices or stopIndex<startIndex => abort"<<endl;
		return;
	};
	nbrSamplesPerSubSegment=stopInSegment-startInSegment+1;
	// Try to access corrections for the given frequency possibly stored in the demodulator class
	if (tryCorrec)									
	...
	{	nbrSamplesPerInterval=nbrSamplesPerHalfPeriod*abs(intervalMode);
		*nbrIntervalsPerSegment=(int)floor((float)(nbrSamplesPerSubSegment/nbrSamplesPerInterval));
	};
	...
	{	*nbrIntervalsPerSegment=1;
		nbrSamplesPerInterval=int(nbrSamplesPerHalfPeriod*(int)(nbrSamplesPerSubSegment/nbrSamplesPerHalfPeriod)+0.5);
	};
	if(nbrSamplesPerInterval==0 || intervalMode==-1)	//1 demodulation over full subsegment
	{	*nbrIntervalsPerSegment=1;
		nbrSamplesPerInterval=nbrSamplesPerSubSegment;
	};
...
//	// cout << "frequency = " << frequency << " samplingTime = " << samplingTime << endl; 
	...
	{//	// cout << "segment = "<< p << endl;
		...
		Q=0;
		j=p*nbrSamplesPerSegment+startInSegment;	// j sample index in ChA and ChB (between 0 to nbrSegments*nbrSamplesPerSegment-1)
...
//	Digital demodulation by direct multiplication of the channel with cos and sin functions (no fourier transform). 
//	This function operates on one segment or on each segment of a sequence. 
//	For each segment, it demodulates only within a specified interval (see startInSegment,stopInsegment below);
//	Within this interval, it demodulates over one or several sub-intervals (see startInSegment,stopInsegment,intervalMode below)
//
// Input parameters:
...
//  startInSegment: index of the first point of the demodulation interval in a segment (0 to nbrSamplesPerSegment-1)
//  stopInSegment: index of the last point of the demodulation interval in a segment (startInSegment to nbrSamplesPerSegment-1)
//		Negative index for startInSegment or startInSegment means nbrSamplesPerSegment-index. 
//		So use startInSegment=0 and stoptInSegment=-1 for the full segment
//		Note that any call with stoptInSegment < startInSegment or startInSegment <0 or stopInSegment>nbrSamplesPerSegment will set *nbrIntervalsPerSegment to 0 and won't return any demodulatio results
...
//	phase: phase reference at the time of the first sample (index 0, not startInSegment). Defines the (omega t + phase) argument of the cos and sin multiplicators.
//	intervalMode: interval code used for demodulation:
//		intervalMode>0 means demodulate over each half-periods if possible (otherwise try case below).
//		intervalMode=0 means 1 interval with length equal to the maximum integer number of half-periods in the segment length (otherwise do case below).
//		intervalMode=-1 means 1 interval with full segment length (allways possible).
//	tryCorrec: boolean telling wether gain, offset and IQ rotation corrections at the given frequency are to be performed (if memorized in the corresponding properties of the Class).
...
// Outputs:
//	nbrIntervalsPerSegment: number of demodulation intervals used per segment: 1 for intervalMode<=0 and max(1,floor(nbrSamplesPerSegment/nbrSamplesPerPeriod/abs(intervalMode)*2) for intervalMode>0.
//	arrayI,arrayQ: flat arrays of the nbrSegments*nbrIntervalsPerSegment I and Q quadratures when averageOnly is false (otherwise the two arrays are left empty).
//	arrayMeanI,arrayMeanQ: flat arrays of the nbrIntervalsPerSegment I and Q values averaged over the nbrSegments segments.
//	* P_Time_us: pointer to the evaluation duration of the function in 탎.
//
void Demodulator::demodulate1ChIQ(double *ChA, double *horPos,int nbrSegments,int nbrSamplesPerSegment,int startInSegment,int stopInSegment,double samplingTime, double frequency, double phase,int intervalMode,bool tryCorrec,bool averageOnly,int *nbrIntervalsPerSegment,double *arrayI,double *arrayQ,double *arrayMeanI,double *arrayMeanQ, double *P_Time_us)
{
	...
	demodulate(false,ChA,ChA,horPos,nbrSegments,nbrSamplesPerSegment,startInSegment,stopInSegment,samplingTime,frequency,phase,intervalMode,tryCorrec,averageOnly,false,nbrIntervalsPerSegment,arrayI,arrayQ,arrayMeanI,arrayMeanQ);
...
// Digital demodulation by direct multiplication of the channels by a cosine (no fourier transform).
// Same definition, inputs, and outputs as demodulate1ChIQ (see above) except for input arrays:
// Input arrays:
//	ChA,ChB: pointers to the arrays in which the I and Q quadratures are stored, respectively.
//

void Demodulator::demodulate2ChIQ(double *ChA,double *ChB, double *horPos, int nbrSegments, int nbrSamplesPerSegment,int startInSegment,int stopInSegment,double samplingTime,double frequency,double phase,int intervalMode,bool tryCorrec,bool averageOnly,int *nbrIntervalsPerSegment,double *arrayI,double *arrayQ,double *arrayMeanI,double *arrayMeanQ,double* P_Time_us)
{ 	// cout << "Demodulator::demodulate2ChIQ: "<<endl;
	LARGE_INTEGER L_Frequency,L_TmpDebut,L_TmpFin;
	QueryPerformanceFrequency(&L_Frequency);
	QueryPerformanceCounter(&L_TmpDebut); //start the timer
	demodulate(true,ChA,ChB,horPos,nbrSegments,nbrSamplesPerSegment,startInSegment,stopInSegment,samplingTime,frequency,phase,intervalMode,tryCorrec,averageOnly,false,nbrIntervalsPerSegment,arrayI,arrayQ,arrayMeanI,arrayMeanQ);
	QueryPerformanceCounter(&L_TmpFin);				//stop the timer...
	*P_Time_us = 1000000.0 *(L_TmpFin.QuadPart - L_TmpDebut.QuadPart) / L_Frequency.QuadPart;//...and calculate elapsed time in 탎
	// cout<<"OK - duration = " <<*P_Time_us<< " microsec." <<endl;
};
...
// Same definition, inputs, and outputs as demodulate1ChIQ (see above), except for the output arrays:
// Output arrays:
...
//
void Demodulator::demodulate1ChAPhi(double *ChA, double *horPos,int nbrSegments,int nbrSamplesPerSegment,int startInSegment,int stopInSegment,double samplingTime, double frequency, double phase,int intervalMode,bool tryCorrec,bool averageOnly,int *nbrIntervalsPerSegment,double *arrayA,double *arrayPhi,double *arrayMeanA,double *arrayMeanPhi,double* P_Time_us)
{ 	// cout << "Demodulator::demodulate1ChAPhi: ";
	LARGE_INTEGER L_Frequency,L_TmpDebut,L_TmpFin;
	QueryPerformanceFrequency(&L_Frequency);
	QueryPerformanceCounter(&L_TmpDebut);			//start the timer
	demodulate(false,ChA,ChA,horPos,nbrSegments,nbrSamplesPerSegment,startInSegment,stopInSegment,samplingTime,frequency,phase,intervalMode,tryCorrec,averageOnly,true,nbrIntervalsPerSegment,arrayA,arrayPhi,arrayMeanA,arrayMeanPhi);
	QueryPerformanceCounter(&L_TmpFin);				//stop the timer...
	*P_Time_us = 1000000.0 *(L_TmpFin.QuadPart - L_TmpDebut.QuadPart) / L_Frequency.QuadPart;//...and calculate elapsed time in 탎
	// cout<<"OK - duration = " <<*P_Time_us<< " microsec." <<endl;
};

// Use demodulate2ChAPhi to demodulate 2 channels I and Q and return the demodulated amplitude A and phase Phi
// Same definition, inputs, and outputs as demodulate2ChIQ (see above), except for the output arrays:
// Output arrays:
//	arrayA,arrayPhi: arrays of the nbrSegments*nbrIntervalsPerSegment amplitudes A and phase phi when averageOnly is false (otherwise the two arrays are left empty) 
//	arrayMeanA,arrayMeanPhi: arrays of the nbrIntervalsPerSegment A and Phi values averaged over the nbrSegments segments
//
void Demodulator::demodulate2ChAPhi(double *ChA, double *horPos,double *ChB, int nbrSegments, int nbrSamplesPerSegment,int startInSegment,int stopInSegment,double samplingTime,double frequency,double phase,int intervalMode,bool tryCorrec,bool averageOnly,int *nbrIntervalsPerSegment,double *arrayA,double *arrayPhi,double *arrayMeanA,double *arrayMeanPhi,double* P_Time_us)
{	// cout << "Demodulator::demodulate2ChAPhi: ";
	LARGE_INTEGER L_Frequency,L_TmpDebut,L_TmpFin;
	QueryPerformanceFrequency(&L_Frequency);
	QueryPerformanceCounter(&L_TmpDebut);			//start the timer
	demodulate(true,ChA,ChB,horPos,nbrSegments,nbrSamplesPerSegment,startInSegment,stopInSegment,samplingTime,frequency,phase,intervalMode,tryCorrec,averageOnly,true,nbrIntervalsPerSegment,arrayA,arrayPhi,arrayMeanA,arrayMeanPhi);
	QueryPerformanceCounter(&L_TmpFin);				//stop the timer...
	*P_Time_us = 1000000.0 *(L_TmpFin.QuadPart - L_TmpDebut.QuadPart) / L_Frequency.QuadPart;//...and calculate elapsed time in 탎
	// cout<<"OK - duration = " <<*P_Time_us<< " microsec." <<endl;
};
