...
def Lorenzian(y0,yAmp,x0,width):
	'''
	Return the pure Lorentzian function y(x)= y0+yAmp/(1.0+pow(2.0*(x-x0)/width,2)).
	'''
...
def cosine(y0,yAmp,x0,period):
	'''
	Return the pure Cosine function function y(x)=y0+yAmp*cos(2*math.pi*(x-x0)/period).
	'''
...
def cosineExp(y0,yAmp,x0,period,xc):
	'''
	Return the pure exponentially decaying cosine function function y(x)=y0+yAmp*cos(2*math.pi*(x-x0)/period)*exp(-x/xc).
	'''
	return lambda x: y0+yAmp*cos(2*math.pi*(x-x0)/period)*exp(-x/xc)

# Finding the closest point to a target y value
def closestXYtoTargetX(x,y,xTarget):
	'''
	Find in the lists x and y the closest XY point i to the point of x coordinate xTarget.
	Return  xi,yi,i,xError
	'''
	position=min(range(len(x)), key=lambda i: abs(x[i]-xTarget))
	return x[position],y[position],position,x[position]-xTarget 	

def closestXYtoTargetY(x,y,yTarget):
	'''
	Find in the lists x and y the closest XY point i to the point of y coordinate yTarget.
	Return  xi,yi,i,yError
	'''
	position=min(range(len(x)), key=lambda i: abs(y[i]-yTarget))
	return x[position],y[position],position,y[position]-yTarget

def firstXCrossTargetYFromAnEnd(y,x=None,yTarget=0,direction='leftToRight'):
	"""
	Finds the first x at which a y list has crossed a yTarget value.
	If direction='leftToRight' the search starts from the begining of x. If direction='rightToLeft' it starts from the end of x.
	Return x[position],y[position],position,y[position]-yTarget,xInterp
	"""
	if x==None: x=range(len(y))
	crossing=False
	xInterp=None
	position=0
	step=+1
	...
	for i in range(1,len(y)):
		...
		if currentDist<=dist: 
			dist=currentDist
			position=i
		if (differences[0]>0 and differences[i]<=0) or (differences[0]<=0 and differences[i]>=0):
			position=i
			crossing=True
			xInterp= x[i-1]-(x[i]-x[i-1])/(differences[i]-differences[i-1])*differences[i-1]
			break
	if direction !='leftToRight':
		x.reverse();y.reverse()
		position=len(y)-position-1
...
def extremum(x,y,minOrMax="max"):
	'''
	Find a point i in lists x,y at which y is minimum or maximum.
	Return xi,yi,i
	''' 
...
def sortV1V2AscendingV1(v1,v2):
	'''
	Sort two lists v1,v2 by increasing element of v1.
	Return sorted v1,sorted v2,initialIndexes where initialIndexes is the list of previous positions before the sort.
	''' 
...
def fitLorentzian(y,x='',initialBase='',initialHeight='',initialPosition='',initialWidth='',direction='',debug=False):
	""" Find the fitting parameters of a lorentzian peak"""
	...
	if initialWidth=='' or initialWidth==0:
		indexLine=closestXYtoTargetX(x,y,initialPosition)[2]		# find the index of the line in the x y lists
		...
		if indexLine!=0:											# find the closest x on the left that corresponds to mid-height	
			#x1=closestXYtoTargetY(x[:indexLine],y[:indexLine],middle)[0]
		...
		if indexLine!=len(x)-1:										# find the closest x on the right that corresponds to mid-height
			#x2=closestXYtoTargetY(x[indexLine:],y[indexLine:],middle)[0]
